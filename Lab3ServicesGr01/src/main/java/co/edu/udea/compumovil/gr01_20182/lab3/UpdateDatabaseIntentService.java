...
public class UpdateDatabaseIntentService extends IntentService  {
    ...
    /**
     ...
     food.setfName(jsonObject.optString("name"));
     food.setfType("Tipo plato: "+(jsonObject.optString("type")));
     food.setfPrice("Precio: $"+(jsonObject.optString("price")));
     food.setfTime("Tiempo de preparación: "+(jsonObject.optString("time")+" min."));
     ListFood.add(food);
     }
     progress.hide();
