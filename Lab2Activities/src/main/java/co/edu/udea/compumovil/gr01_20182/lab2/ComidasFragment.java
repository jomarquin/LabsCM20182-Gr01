...
public class ComidasFragment extends Fragment {
    ...
    View view;
    EditText fieldName, fieldPrice, fieldTime;
    Button btnRegister;
    ...
    private void registerPlates() {
        ...
        values.put(Utilities.FIELD_P_NAME, fieldName.getText().toString());
        if (rb_plateIn.isChecked()){
         values.put(Utilities.FIELD_P_TYPE, "Entrada");
         }else {
         values.put(Utilities.FIELD_P_TYPE, "Plato Fuerte");
         }
        values.put(Utilities.FIELD_P_PRICE, fieldPrice.getText().toString());
        values.put(Utilities.FIELD_P_TIME, fieldTime.getText().toString());

        long idResult = db.insert(Utilities.TABLE_PLATES, Utilities.FIELD_P_TIME, values);
        Toast.makeText(getContext().getApplicationContext(), "Plato Registrado: "+idResult, Toast.LENGTH_SHORT).show();
        db.close();
        fieldName.setText("");
        fieldPrice.setText("");
        fieldTime.setText("");
